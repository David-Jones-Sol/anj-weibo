...
#import "HotCommentViewController.h"
...
@interface HotCommentViewController ()

@end

@implementation HotCommentViewController

@synthesize statuses = _statuses;
- (IBAction)refresh:(UIBarButtonItem *)sender {
   [engine getHotCommnetDaily:50];    
}
...
- (void)viewDidLoad
{
    ...
    if (!accessToken) {
       ...
       NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
        [center addObserver:self selector:@selector(didGetHotCommentDaily:) name:MMSinaGotHotCommentDaily object:nil];
    }
    
}
-(void)didGetHotCommentDaily:(NSNotification*)sender
{
...
- (void)viewDidUnload
{
    [super viewDidUnload];
     NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
    [center removeObserver:self name:MMSinaGotHotCommentDaily object:nil];
}
